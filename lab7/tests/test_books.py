...
def test_any_auth_user_can_post(client: TestClient, bob_headers):
    assert client.post("/books/", json=FIXTURE, headers=bob_headers).status_code == 201


def test_list_structure(client: TestClient, alice_headers):
...
def test_filter_on_loan(client: TestClient, alice_headers):
